...
static AM_INLINE AM_ErrorCode_t vlfend_get_dev(int dev_no, AM_FEND_Device_t **dev)
{
    ...
    {
        DTV_LOGE(TAG, "invalid vlfrontend device number %d, must in(%d~%d)",
...
static AM_INLINE AM_ErrorCode_t vlfend_get_opened_dev(int dev_no, AM_FEND_Device_t **dev)
{
...
AM_ErrorCode_t AM_VLFEND_Open(int dev_no, const AM_FEND_OpenPara_t *para)
{
...

    {
        ...
        if (rc != 0)
            DTV_LOGE(TAG, "sigaction: err=%d", errno);
    }
...
AM_ErrorCode_t AM_VLFEND_SetMode(int dev_no, int mode)
{
    ...
    DTV_LOGI(TAG, "AM_VLFEND_SetMode %d", mode);

    if (!dev->drv->set_mode)
    {
        DTV_LOGE(TAG, "vlfronend %d no not support set_mode", dev_no);
...
AM_ErrorCode_t AM_VLFEND_SetPara(int dev_no, const struct dvb_frontend_parameters *para)
{
    ...
    DTV_LOGI(TAG, "AM_VLFEND_SetPara\n");

    AM_TRY(vlfend_get_opened_dev(dev_no, &dev));

    if (!dev->drv->set_para)
    {
        DTV_LOGE(TAG, "vlfronend %d no not support set_para", dev_no);
...
AM_ErrorCode_t AM_VLFEND_GetPara(int dev_no, struct dvb_frontend_parameters *para)
{
    ...
    if (!dev->drv->get_para)
    {
        DTV_LOGE(TAG, "vlfronend %d no not support get_para", dev_no);
...
AM_ErrorCode_t AM_VLFEND_SetProp(int dev_no, const struct dtv_properties *prop)
{
    ...
    if (!dev->drv->set_prop)
    {
        DTV_LOGE(TAG, "vlfronend %d no not support set_prop", dev_no);
...
AM_ErrorCode_t AM_VLFEND_GetProp(int dev_no, struct dtv_properties *prop)
{
    ...
    if (!dev->drv->get_prop)
    {
        DTV_LOGE(TAG, "vlfronend %d no not support get_prop", dev_no);
...
AM_ErrorCode_t AM_VLFEND_GetStatus(int dev_no, fe_status_t *status)
{
    ...
    {
        DTV_LOGE(TAG, "vlfronend %d no not support get_status", dev_no);
 ...
 *   - Other value see(am_fend.h)
 */
AM_ErrorCode_t AM_VLFEND_GetCallback(int dev_no, AM_FEND_Callback_t *cb, void **user_data, int *user_data_len)

{
    ...
    if (user_data && user_data_len)
    {
        *user_data      = dev->user_data;
        *user_data_len  = dev->user_data_len;
    }
...
AM_ErrorCode_t AM_VLFEND_Lock(int dev_no, const struct dvb_frontend_parameters *para, fe_status_t *status)
{
    ...
    DTV_LOGI(TAG, "AM_VLFEND_Lock line:%d\n", __LINE__);
    ...
    DTV_LOGI(TAG, "AM_VLFEND_Lock line:%d,ret:%d\n", __LINE__, ret);
...
AM_ErrorCode_t AM_VLFEND_SetThreadDelay(int dev_no, int delay)
{
    ...
    if (dev->thread == pthread_self())
    {
        DTV_LOGI(TAG, "cannot invoke AM_VLFEND_Lock in callback");
...
AM_ErrorCode_t AM_VLFEND_GetAtvStatus(int dev_no, atv_status_t *atv_status)
{
    ...
    {
        DTV_LOGI(TAG, "vlfronend %d no not support get atv status", dev_no);
...
AM_ErrorCode_t AM_VLFEND_DetectStandard(int dev_no)
{
    ...
    {
        pthread_mutex_unlock(&dev->lock);
        DTV_LOGE(TAG, "ioctl V4L2_DETECT_AUDIO failed, errno: %s", strerror(errno));
