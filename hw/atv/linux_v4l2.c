 ...
 * \modified by Robin
 * \date 2023-06-12: porting for aml dtv turnkey solution
 ***************************************************************************/
...
static AM_ErrorCode_t v4l2_open(AM_FEND_Device_t *dev, const AM_FEND_OpenPara_t *para)
{
    ...
    if (fd == -1)
    {
        DTV_LOGI(TAG, "cannot open %s, error:%s", name, strerror(errno));
        return AM_FEND_ERR_CANNOT_OPEN;
    }

    DTV_LOGI(TAG, "open %s, %d.", name, fd);
...
static AM_ErrorCode_t v4l2_set_mode(AM_FEND_Device_t *dev, int mode)
{
    ...
    {
        DTV_LOGI(TAG, "ioctl V4L2_SET_MODE failed, error:%s", strerror(errno));
        return AM_FAILURE;
    }
...
static AM_ErrorCode_t v4l2_set_para(AM_FEND_Device_t *dev, const struct dvb_frontend_parameters *para)
{
    ...
    DTV_LOGI(TAG, "ioctl V4L2_SET_FRONTEND\n");

    if (ioctl(fd, V4L2_SET_FRONTEND, &v4l2_para) == -1)
    {
        DTV_LOGI(TAG, "ioctl V4L2_SET_FRONTEND failed, error:%s", strerror(errno));

        return AM_FAILURE;
    }

    DTV_LOGI(TAG, "ioctl V4L2_SET_FRONTEND success \n");
...
static AM_ErrorCode_t v4l2_get_para(AM_FEND_Device_t *dev, struct dvb_frontend_parameters *para)
{
    ...
    {
        DTV_LOGI(TAG, "ioctl V4L2_GET_FRONTEND failed, error:%s", strerror(errno));
        return AM_FAILURE;
    }
...
static AM_ErrorCode_t v4l2_set_prop (AM_FEND_Device_t *dev, const struct dtv_properties *prop)
{
    ...
    if (property == NULL)
    {
        DTV_LOGI(TAG, "malloc failed, error:%s", strerror(errno));
        return AM_FAILURE;
    }
    ...
    DTV_LOGI(TAG, "V4L2_SET_PROPERTY cmd = 0x%x", prop->props->cmd);

    if (ioctl(fd, V4L2_SET_PROPERTY, &v4l2_prop) == -1)
    {
        DTV_LOGI(TAG, "ioctl V4L2_SET_PROPERTY failed, error:%s", strerror(errno));
        return AM_FAILURE;
    }
...
static AM_ErrorCode_t v4l2_get_prop (AM_FEND_Device_t *dev, struct dtv_properties *prop)
{
    ...
    DTV_LOGI(TAG, "V4L2_GET_PROPERTY cmd = 0x%x", prop->props->cmd);

    if (ioctl(fd, V4L2_GET_PROPERTY, &v4l2_prop) == -1)
    {
        DTV_LOGI(TAG, "ioctl V4L2_GET_PROPERTY failed, error:%s", strerror(errno));
        return AM_FAILURE;
    }
...
static AM_ErrorCode_t v4l2_get_status(AM_FEND_Device_t *dev, fe_status_t *status)
{
    ...
    {
        DTV_LOGI(TAG, "ioctl V4L2_READ_STATUS failed, error:%s", strerror(errno));
        return AM_FAILURE;
    }
...
static AM_ErrorCode_t v4l2_wait_event(AM_FEND_Device_t *dev, struct dvb_frontend_event *evt, int timeout)
{
    ...
    {
        DTV_LOGI(TAG, "ioctl V4L2_GET_EVENT failed, error:%s", strerror(errno));
        return AM_FAILURE;
    }
...
static AM_ErrorCode_t v4l2_get_tune_status(AM_FEND_Device_t *dev, atv_status_t *atv_status)
{
    ...
    {
        DTV_LOGI(TAG, "ioctl FE_READ_ANALOG_STATUS failed, errno: %s", strerror(errno));
        return AM_FAILURE;
    }
