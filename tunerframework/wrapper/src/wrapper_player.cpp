...
typedef struct
{
    ...
    jobject playerWeakRefPcrFilter;
    ...
    if (wp_player_av_status == NULL)
    {
        ...
        if (wp_player_av_status != NULL)
        {
            for (av_path = 0; av_path < player_paths; av_path++)
            {
                ...
                wp_player_av_status[av_path].playerWeakRefPcrFilter = NULL;
            }
        }
...
S8BIT Wrapper_Player_Destroy(jni_asplayer_handle handle)
{
    ...
    if (wp_player_av_status[av_path].playerWeakRefPcrFilter != NULL)
    {
        Am_filter_close(wp_player_av_status[av_path].playerWeakRefPcrFilter);
        wp_player_av_status[av_path].playerWeakRefPcrFilter = NULL;
    }
    ...
    {
        ret = JNI_ASPLAYER_OK;
        ALOGI("%s : handle = %u, av_path: %u", __FUNCTION__, handle, av_path);
    }
    else
    {
...
S8BIT Wrapper_Player_SetPlayerNo(U8BIT av_path, U8BIT player_no)
{
    ...
    {
        wp_player_av_status[av_path].player_no = player_no;
    }
    return 0;
}
...
S8BIT Wrapper_Player_SetVideoParams(jni_asplayer_handle handle, jni_asplayer_video_params *video_params, WRAPPER_PLAYER_VIDEO_STREAM_TYPE format)
{
    ...
    {
        video_params->filterId = player_GetAVFilterId(false, false, video_params->pid, format, WP_AUDIO_STREAM_TYPE_UNDEFINED, handle);
        video_params->avSyncHwId = player_GetAVSyncHwId(false, false, handle);
...
S8BIT Wrapper_Player_SetAudioParams(jni_asplayer_handle handle, jni_asplayer_audio_params *audio_params, WRAPPER_PLAYER_AUDIO_STREAM_TYPE format)
{
    ...
    {
        audio_params->filterId = player_GetAVFilterId(true, false, audio_params->pid, WP_VIDEO_STREAM_TYPE_UNDEFINED, format, handle);
        audio_params->avSyncHwId = player_GetAVSyncHwId(true, false, handle);
...
S8BIT Wrapper_Player_SetPcrPid(jni_asplayer_handle handle, U16BIT pcr_pid)
{
    ...
    U8BIT av_path = Wrapper_Player_GetPlayerPathByHandle(handle);
    if (wp_player_av_status[av_path].tunerType == WP_TUNER_TYPE_LIVE_0 ||
        wp_player_av_status[av_path].tunerType == WP_TUNER_TYPE_LIVE_1 ||
        wp_player_av_status[av_path].tunerType == WP_TUNER_TYPE_LIVE_2)
    {
        jobject pcrFilter;
        if (wp_player_av_status[av_path].playerWeakRefPcrFilter != NULL)
        {
        ...
        Am_filter_callback pcrFilterCallback = player_PcrFilterCallback;
        pcrFilter = Am_tuner_openFilter(wp_player_av_status[av_path].playerClient, MAIN_TYPE_TS, SUBTYPE_PCR, WRAPPER_PLAYER_BUFFER_SIZE_PCR_DEFAULT, (long)pcrFilterCallback, 0);

        if (NULL == pcrFilter) {
            ALOGI("%s : get pcr filter fail, Filter is null", __FUNCTION__);
            playerFailLeave(attached);
            return ret;
        }

        jobject pcrFilterConfigurationObject = filter_utils_getTsFilterConfiguration(env, pcr_pid);
        if (NULL == pcrFilterConfigurationObject) {
            ALOGI("%s : get pcr filter fail, get AVTsFilterConfigurationObject object error", __FUNCTION__);
            env->DeleteWeakGlobalRef(pcrFilter);
            playerFailLeave(attached);
            return ret;
        }
        int result = Am_filter_configure(pcrFilter, pcrFilterConfigurationObject);
        ALOGI("%s : pcr filter configure result : %d", __FUNCTION__, result);
        //3.start filter
        result = Am_filter_start(pcrFilter);
        ALOGI("%s : pcr filter start result : %d", __FUNCTION__, result);
        //4.get filter Id
        int filterId = Am_filter_getId(pcrFilter);
        ALOGI("%s : pcr filter Id : %d", __FUNCTION__, filterId);
        wp_player_av_status[av_path].playerWeakRefPcrFilter = pcrFilter;
        ret = JNI_ASPLAYER_OK;
    }
...
S8BIT Wrapper_Player_GetVideoInfo(jni_asplayer_handle handle, jni_asplayer_video_info *pInfo)
{
    ...
    {
        ret = JNI_ASPLAYER_OK;
        ALOGI("%s : W/H[%ux%u], VF:%u, Framerate:%u, Ratio:%u, handle = %u", __FUNCTION__,
            pInfo->width, pInfo->height, pInfo->vfType, pInfo->framerate, pInfo->aspectRatio, handle);
    }
    else
    {
...
S8BIT Wrapper_Player_SetADParams(jni_asplayer_handle handle, jni_asplayer_audio_params *ad_params, WRAPPER_PLAYER_AUDIO_STREAM_TYPE format)
{
    ...
    if (ad_params != NULL)
    {
        ad_params->filterId = player_GetAVFilterId(true, true, ad_params->pid, WP_VIDEO_STREAM_TYPE_UNDEFINED, format, handle);
        ad_params->avSyncHwId = player_GetAVSyncHwId(true, true, handle);
...
jni_asplayer_handle Wrapper_Player_GetPlayerHandleByPath(U8BIT av_path)
{
...
static int player_GetAVFilterId(bool isAudio, bool isAD, int pid, int videoStreamType, int audioStreamType, jni_asplayer_handle handle)
{
    ALOGI("start:%s, isAudio: %d, isAD: %d, pid: %d, video type: %d, audio type: %d ", __FUNCTION__, isAudio, isAD, pid, videoStreamType, audioStreamType);
    ...
    if (NULL == avFilter) {
        ALOGI("%s : get filter fail, Filter is null", __FUNCTION__);
        playerFailLeave(attached);
        return INVALID_VALUE;
    }
    ...
    if (NULL == tsFilterConfigurationObject) {
        ALOGI("%s : get filter fail, get AVTsFilterConfigurationObject object error", __FUNCTION__);
...
static int player_GetAVSyncHwId(bool isAudio, bool isAD, jni_asplayer_handle handle)
{
    ALOGI("start:%s, isAudio: %d, isAD: %d", __FUNCTION__, isAudio, isAD);
    ...
    int avSyncHwId = INVALID_VALUE;
    U8BIT av_path = Wrapper_Player_GetPlayerPathByHandle(handle);
    if (true == isAudio)
    {
        if (true == isAD)
            avSyncHwId = Am_tuner_getAvSyncHwId(wp_player_av_status[av_path].playerClient, wp_player_av_status[av_path].playerWeakRefADFilter);
        else
            avSyncHwId = Am_tuner_getAvSyncHwId(wp_player_av_status[av_path].playerClient, wp_player_av_status[av_path].playerWeakRefAudioFilter);
    }
    else
    {
