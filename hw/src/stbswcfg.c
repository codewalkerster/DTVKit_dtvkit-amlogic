...
void STB_LoadSwConfigJsonDB()
{
    ...
    {
        APCFG_AddFilePathForDtvKitDataFile(strDataPath,128,TV_JSONFILE);
        if ((access(strDataPath, F_OK)) != 0)
        {
            APCFG_AddFilePathForDtvKitConfigFile(strCfgPath,128,CONFIG_JSONFILE);
...
int  STB_SetSystemStartingMode(char* system_starting_mode)
{
    ...
    APCFG_AddFilePathForDtvKitDataFile(strDataPath,128,g_config_json_file);
    TvConfigSyncJsonDBFromCache(strDataPath, &tv_config);
